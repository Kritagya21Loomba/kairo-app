...
export function AppNav() {
  ...
  return (
    <>
      <header
        className="sticky top-0 z-50 border-b-2 border-[var(--border-color)]"
        style={{ backgroundColor: "var(--bg-card)" }}
      >
        <div
          className="h-0.5 w-full"
          style={{ background: "linear-gradient(90deg, var(--accent) 0%, var(--accent-alt) 50%, var(--accent) 100%)" }}
        />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <Link href="/" className="border-l-2 border-[var(--border-color)] pl-4 flex items-baseline gap-2 hover:opacity-80 transition-opacity">
              ...
              <span className="text-xs font-mono-data ml-1 px-1.5 py-0.5 border rounded"
                style={{ color: "var(--text-muted)", borderColor: "var(--border-color)", fontSize: "0.6rem" }}>v3.0</span>
            </Link>
          </div>
          ...
          <nav className="flex items-center gap-2">
            ...
            {/* + Log button: only visible when signed in */}
            {!loading && user && (
              <button
      ...
      {/* Onboarding modal — only after loading resolves AND user is fresh with no username */}
      <AnimatePresence>
